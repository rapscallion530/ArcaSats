...
Multi-user (owner-scoped), in-app node settings (Sparrow-style Tor toggle), node status widget, cross-account basis carry + per-transfer carry on/off, account/wallet/tx edit+delete, auto script-type detection, 78 tests.
...
6. ❌ **UTXO-level lot engine — deliberately deferred.** Rationale: it's a major architectural rewrite
   ...
   destabilize a working, 83-test app, for marginal benefit to a DCA/low-volume holder. Rev. Proc. 2024-28's
