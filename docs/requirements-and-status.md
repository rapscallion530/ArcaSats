...
Single-user (optional app-wide password lock), in-app node settings (Sparrow-style Tor toggle), node status widget, cross-account basis carry + per-transfer carry on/off, account/wallet/tx edit+delete, auto script-type detection, KYC origin on cost-basis lots + KYC-aware disposal priority, address-based fuzzy-hop detection, price-source registry abstraction, separate node/mempool connections (each tested independently; mempool over Tor), unified "Balance" (holding_sats == account balance; BTC fees reduce both), lossless CSV mapping (raw-row stash + custodian basis/acq-date + CSV→wallet linkage) with an expandable per-tx detail view, native-window desktop launcher (pywebview; close window to quit; browser-tab fallback + logging if no window), bundled self-managing Tor (desktop app runs its own verified Tor so .onion works with no second app; auto-update/CVE path; single-instance guard), account KYC-status dropdown (KYC/Non-KYC; legacy free-text labels migrated to notes), master ledger (all transactions across accounts/wallets, filters + CSV), KYC taint on entry (non-KYC coins entering a KYC account become KYC; one-way ratchet), testnet derivation coverage (live-validated on blockstream mainnet+testnet), raw-tx parser fallback so xpub sync works against non-verbose Electrum servers, $0-proceeds disposal readiness flag, combined all-accounts 8949/Schedule D (per-account results summed, by-account + by-KYC, CSV), 225 tests.
